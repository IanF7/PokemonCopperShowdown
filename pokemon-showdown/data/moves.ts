...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	bounce: {
		...
		condition: {
			...
			onInvulnerability(target, source, move) {
				if (['gust', 'twister', 'skyuppercut', 'thunder', 'hurricane', 'smackdown', 'thousandarrows', 'telekineticslam'].includes(move.id)) {
					return;
				}
				return false;
			},
	...
	smackdown: {
		...
		condition: {
			...
			onStart(pokemon) {
				let applies = false;
				if (pokemon.hasType('Flying') || pokemon.hasAbility('levitate') || pokemon.hasAbility('eelevate')) applies = true;
	...
	frostbite: {
		...
		basePowerCallback(pokemon, target, move) {
			if (target.status) {
				this.debug('BP doubled for target status');
				return move.basePower * 2;
			}
			return move.basePower;
		},
	...
	vampiricbite: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, bite: 1, heal: 1, metronome: 1 },
		drain: [1, 2],
		target: "normal",
		type: "Dark",
		contestType: "Clever",
	...
	equilibriumpulse: {
		...
		pp: 5,
		priority: 0,
		flags: { protect: 1, mirror: 1, pulse: 1, metronome: 1 },
		secondary: {
			chance: 30,
			volatileStatus: 'flinch',
		},
	...
	mysticward: {
		...
		flags: { snatch: 1, bypasssub: 1, metronome: 1 },
		onHitSide(side) {
			const boosts: SparseBoostsTable = {spd: 1};
			if (this.field.isTerrain('mistyterrain')) boosts.def = 1;
